...
def load_obras(path: str | Path) -> list[Obra]:
    ...
    if path.suffix.lower() != ".csv":
        raise ValueError(
            "La V0 usa CSV para que GitHub pueda versionar y comparar los datos. "
            "El soporte directo de XLSX se agregará en una versión posterior."
        )
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as fh:
        ...
        required = {
            ...
            "auditor_responsable",
            "duracion_horas",
            "supervisor_preferente_id",
            "supervisores_alternativos_ids",
            "contratista_id",
            "prioridad",
        }
        ...
        if missing:
            raise ValueError(
                f"Faltan columnas obligatorias en {path}: {sorted(missing)}"
            )
            ...
            obras.append(
                Obra(
                    ...
                    auditor_responsable=str(
                        row["auditor_responsable"]
                    ).strip(),
                    duracion_minutos=int(round(duracion_horas * 60)),
                    latitud=latitud,
                    longitud=longitud,
                    supervisor_preferente_id=_optional_text(
                        row.get("supervisor_preferente_id")
                    ),
                    supervisores_alternativos_ids=_parse_ids(
                        row.get("supervisores_alternativos_ids")
                    ),
                    contratista_id=_optional_text(row.get("contratista_id")),
                    prioridad=int(prioridad_text or 3),
                    tipo_inspeccion=str(
                        row.get("tipo_inspeccion", "")
                    ).strip(),
                )
            )
